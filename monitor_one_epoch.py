...
def main():
    ...
    global args, best_robust, model_idx, P
    ...
    model.load_state_dict(torch.load(os.path.join(args.save_dir,  '126.pt')))
    args.start_epoch = 63
    ...
    # DLDR sampling
    torch.save(model.state_dict(), os.path.join(args.save_dir,  'mid00.pt'))
    model_idx += 1


    # train for one epoch
    print('current lr {:.5e}'.format(optimizer.param_groups[0]['lr']))
    train(train_loader, val_loader, model, criterion, optimizer, args.start_epoch + 1)
...
def train(train_loader, val_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % 5 == 0:
            torch.save(model.state_dict(), os.path.join(args.save_dir,  'psgd_mid_' + str(args.lr) +  '_' + str(i) +  '.pt'))
            # model_idx += 1
